...
import Icon from "../components/Icon"; // Ensure Icon is imported
...
const ValuationChart = ({ totalValue }) => {
  // Mock trend data for visualization (replace with real historical data if available)
  ...
  return (
    <div className="bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark rounded-xl p-6 relative overflow-hidden h-full">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h3 className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-widest">Financial Health</h3>
...
const WarrantyWidget = ({ assets }) => (
  <div className="bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark rounded-xl overflow-hidden flex flex-col h-full">
    <div className="p-5 border-b border-slate-200 dark:border-border-dark flex justify-between items-center bg-slate-50/50 dark:bg-white/5">
      ...
      <span className="text-xs font-bold bg-red-100 text-red-600 px-2 py-1 rounded-md">{assets.length} Alerts</span>
    </div>
    <div className="p-0 flex-1 overflow-y-auto custom-scrollbar h-[250px]">
        {assets.length === 0 ? (
            <div className="p-8 text-center text-slate-400 text-sm">No warranties expiring soon.</div>
        ) : (
            <table className="w-full text-left">
                ...
                {assets.map((item, i) => (
                    ...
                    <td className="px-5 py-3 text-right">
                        ...
                        item.daysLeft < 15 ? 'text-red-600 bg-red-50 dark:bg-red-500/10' : 'text-amber-600 bg-amber-50 dark:bg-amber-500/10'
                        }`}>
                        {item.daysLeft} Days
                        </span>
                    </td>
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const assigned = assetList.filter(a => ['Assigned', 'Active'].includes(a.status) || a.deviceDetails?.currentStatus === 'Assigned').length;
        const maintenance = assetList.filter(a => ['Maintenance', 'Repair'].includes(a.status) || a.deviceDetails?.currentStatus === 'Maintenance').length;
        const unassigned = assetList.filter(a => ['Unassigned', 'In Stock'].includes(a.status) || a.deviceDetails?.currentStatus === 'Unassigned').length;
        ...
        // 2. Financial Valuation (Sum of Price)
        const valuation = assetList.reduce((sum, asset) => sum + (Number(asset.price) || 0), 0);
        ...
        assetList.forEach(asset => {
            if (asset.purchaseDate) {
                ...
                // Warranty Logic (Assuming 1 year warranty for example)
                ...
                if (daysLeft > 0 && daysLeft <= 60) {
                    warrantyList.push({
                        id: asset.assetCode,
                        name: asset.assetName || asset.assetType,
                        daysLeft
                    });
                }
            }
        ...
        // Format Aging Data for Widget
        setAgingData([
            { label: "< 1 Year", count: agingCounts['<1'], percent: (agingCounts['<1']/total)*100, color: "bg-emerald-500" },
            { label: "1-3 Years", count: agingCounts['1-3'], percent: (agingCounts['1-3']/total)*100, color: "bg-blue-500" },
            { label: "3-5 Years", count: agingCounts['3-5'], percent: (agingCounts['3-5']/total)*100, color: "bg-amber-500" },
            { label: "> 5 Years", count: agingCounts['>5'], percent: (agingCounts['>5']/total)*100, color: "bg-red-500" },
        ]);

        setExpiringWarranties(warrantyList.sort((a,b) => a.daysLeft - b.daysLeft));
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-slate-50/50 dark:bg-background-dark/20">
          ...
          <div className="flex justify-between items-end mb-8">
            ...
            <div className="flex gap-2">
                <button onClick={() => navigate('/reports')} className="px-4 py-2 bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark rounded-lg text-sm font-bold shadow-sm hover:bg-slate-50 transition-all flex items-center gap-2">
                ...
                <div className="flex-1 space-y-4 overflow-y-auto custom-scrollbar pr-2">
                    {requestStats.pending > 5 && (
                        <div className="flex gap-4 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20">
                            ...
                            <div>
                                <h4 className="text-sm font-bold text-slate-900 dark:text-white">Request Backlog</h4>
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{requestStats.pending} requests pending. Approval required.</p>
                            </div>
                        </div>
                    )}
                    <div className="flex gap-4 p-4 rounded-xl bg-slate-50 dark:bg-background-dark border border-slate-200 dark:border-border-dark">
                        ...
                        <div>
                            <h4 className="text-sm font-bold text-slate-900 dark:text-white">Data Sync Active</h4>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Real-time stats are up to date.</p>
                        </div>
                    </div>
                </div>
             </div>
